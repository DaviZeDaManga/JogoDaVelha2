  ...
  useEffect(() => {
    ...
    for (let jogo of jogos) {
      for (let linha of linhas) {
        ...
        if (
          jogo[a] === jogo[b] && jogo[b] === jogo[c] && jogo[c] === jogo[a] 
        ) {
    ...
    <div className="JogoDaVelha2">
      ...
      <section className="jogo">
        {jogoprincipal.map((item, index) => (
          <div key={index} className={`cards-jogo_principal ${item == "x" && "x"} ${ item == "o" && "o"} ${jogandoojogo == index && "jogando"}`}>
